...
// Relative regex
export function getTomorrowAndDayAfterTomorrowRegex() {
  return /\b(day\s+after\s+tomorrow|tomorrow)\b/i

}
export function getRelativeTimeRegex() {
  return /\b(day\s+after\s+tomorrow|tomorrow|in\s+\d+\s+\w+)\s*(?:at\s*)?((?:1[0-2]|0?[1-9])(?::([0-5][0-9]))?\s*(am|pm))?\b/i;
}


export function getInOrAtRegex() {
  return /^(?:at|in)\s+(\d+(?:\.\d+)?)\s+(milliseconds?|ms|seconds?|Seconds?|Sec|sec|s|minutes?|min|Minutes?|Min|m|hours?|hr|Hrs?|Hr|h|days?|day|Days?|Day|d|weeks?|week|Weeks?|Week|w|years?|year|Years?|Year|y)/i
}
export function getInOrAtWithTimeRegex() {
  return /^(?:at|in)\s+(1[0-2]|0?[1-9])(?::([0-5][0-9]))?\s*(am|pm)?/i
}
...
//  dd/mm/yyyy or dd-mm-yyyygetInRegex
...
export function getTimeRegx() {
  return /\b(1[0-2]|0?[1-9])(?::([0-5][0-9]))?\s*(am|pm)\b/i;
